...
namespace GameFrameworkAsset
{
    ...
    /// 文件系统管理器。
    /// </summary>
    internal sealed class FileSystemManager : IFileSystemManager
    {
        ...
        public IFileSystem CreateFileSystem(string fullPath, FileSystemAccess access, int maxFileCount, int maxBlockCount)
        {
            ...
            {
                throw new Exception("Access read is invalid.");
            }
            ...
            AFileSystemStream fileSystemStream = m_FileSystemHelper.CreateFileSystemStream(fullPath, access, true);
            if (fileSystemStream == null)
            {
                throw new Exception($"Create file system stream for '{fullPath}' failure.");
            }
        ...
        public IFileSystem LoadFileSystem(string fullPath, FileSystemAccess access)
        {
            ...
            AFileSystemStream fileSystemStream = m_FileSystemHelper.CreateFileSystemStream(fullPath, access, false);
            if (fileSystemStream == null)
            {
                throw new Exception($"Create file system stream for '{fullPath}' failure.");
            }
