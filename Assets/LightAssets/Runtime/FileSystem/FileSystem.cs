...
namespace LightAssets 
{
    ...
    internal sealed partial class FileSystem : IFileSystem
    {
        ...
        private readonly AFileSystemStream m_Stream;
        private readonly Dictionary<string, int> m_FileDatas;
        private readonly List<BlockData> m_BlockDatas;
        private readonly Dictionary<int,LinkedList<int>> m_FreeBlockIndexes;
        private readonly Queue<LinkedListNode<int>> m_FreeBlockNodeCaches;
        ...
        private FileSystem(string fullPath, FileSystemAccess access, AFileSystemStream stream)
        {
            ...
            m_FreeBlockIndexes = new Dictionary<int, LinkedList<int>>();
            m_FreeBlockNodeCaches = new Queue<LinkedListNode<int>>();
        ...
        public static FileSystem Create(string fullPath, FileSystemAccess access, AFileSystemStream stream, int maxFileCount, int maxBlockCount)
        {
            if (maxFileCount <= 0)
            {
            ...
            Utility.Marshal.StructureToBytes(fileSystem.m_HeaderData, HeaderDataSize, s_CachedBytes, 0);
        ...
        public static FileSystem Load(string fullPath, FileSystemAccess access, AFileSystemStream stream)
        {
            FileSystem fileSystem = new FileSystem(fullPath, access, stream);

            stream.Read(s_CachedBytes, 0, HeaderDataSize);
            fileSystem.m_HeaderData = Utility.Marshal.BytesToStructure<HeaderData>(HeaderDataSize, s_CachedBytes ,0);
            ...
            for (int i = 0; i < fileSystem.m_HeaderData.BlockCount; i++)
            {
                ...
                BlockData blockData = Utility.Marshal.BytesToStructure<BlockData>(BlockDataSize, s_CachedBytes, 0);
            ...
            for (int i = 0; i < fileSystem.m_BlockDatas.Count; i++)
            {
                ...
                {
                    fileSystem.AddFreeBlockIndex(blockData.Length,i);
                }
            }
        ...
        public bool DeleteFile(string name)
        {
            ...
            {
                AddFreeBlockIndex(blockData.Length,blockIndex);
        ...
        private void ProcessWriteFile(string name, bool hasFile, int oldBlockIndex, int blockIndex, int length)
        {
            ...
            if (hasFile)
            {
                ...
                {
                    AddFreeBlockIndex(oldBlockData.Length,oldBlockIndex);
        ...
        private bool TryCombineFreeBlocks(int freeBlockIndex)
        {
            ...
            foreach (KeyValuePair<int, LinkedList<int>> blockIndexes in m_FreeBlockIndexes)
            {
            ...
            RemoveFreeBlockIndex(freeBlockData.Length, freeBlockIndex);
            if (previousFreeBlockIndex >= 0)
            {
                BlockData previousFreeBlockData = m_BlockDatas[previousFreeBlockIndex];
                RemoveFreeBlockIndex(previousFreeBlockData.Length, previousFreeBlockIndex);
                ...
                AddFreeBlockIndex(0,previousFreeBlockIndex);
            ...
            if (nextFreeBlockIndex >= 0)
            {
                ...
                RemoveFreeBlockIndex(nextFreeBlockData.Length, nextFreeBlockIndex);
                ...
                AddFreeBlockIndex(0, nextFreeBlockIndex);
                WriteBlockData(nextFreeBlockIndex);
            }

            m_BlockDatas[freeBlockIndex] = freeBlockData;
            AddFreeBlockIndex(freeBlockData.Length,freeBlockIndex);
        ...
        private int AllocBlock(int length)
        {
            ...
            int lengthFound = -1;
            LinkedList<int> lengthRange = default(LinkedList<int>);
            foreach (KeyValuePair<int, LinkedList<int>> i in m_FreeBlockIndexes)
            {
                if (i.Key < length)
                {
                    continue;
            ...
            if (lengthFound >= 0)
            {
                ...
                int blockIndex = lengthRange.First.Value;
                RemoveFreeBlockIndex(lengthFound, blockIndex);
                if (lengthFound > length)
                {
                    ...
                    AddFreeBlockIndex(deltaLength,anotherBlockIndex);
        ...
        private void AddFreeBlockIndex(int dataLength, int index) 
        {
            if (!m_FreeBlockIndexes.TryGetValue(dataLength, out var linkedList)) {
                linkedList = new LinkedList<int>();
                linkedList.AddFirst(GetCacheNode(index));
                m_FreeBlockIndexes.Add(dataLength, linkedList);
            }
            else {
                if (linkedList.First == null) {
                    ...
                    linkedList.AddAfter(current, GetCacheNode(index));
                }

                m_FreeBlockIndexes[dataLength] = linkedList;
            }
        }

        private LinkedListNode<int> GetCacheNode(int value) {
            if (m_FreeBlockNodeCaches.Count > 0) {
                LinkedListNode<int> node = m_FreeBlockNodeCaches.Dequeue();
                node.Value = value;
                return node;
            }
        ...
        private bool RemoveFreeBlockIndex(int dataLength, int index) 
        {
            if (m_FreeBlockIndexes.TryGetValue(dataLength, out var linkedList))
            {
                LinkedListNode<int> current = linkedList.First;
                while (current != null) 
                {
                    if (current.Value.Equals(index)) 
                    {
                        ...
                        if (current == linkedList.First) 
                        {
                            ...
                            //下一个节点为空 代表整个链表都可以被移除
                            if (next == null) {
                                m_FreeBlockIndexes.Remove(dataLength);
                            }
                            //把下一个节点,变成头部节点
                            else {
                        ...
                        m_FreeBlockNodeCaches.Enqueue(current);
                        return true;
                    }
        ...
        private void WriteHeaderData()
        {
            ...
            Utility.Marshal.StructureToBytes(m_HeaderData, HeaderDataSize, s_CachedBytes, 0);
        ...
        {
            Utility.Marshal.StructureToBytes(m_BlockDatas[blockIndex], BlockDataSize, s_CachedBytes, 0);
        ...
        private StringData ReadStringData(int stringIndex)
        {
            ...
            return Utility.Marshal.BytesToStructure<StringData>(StringDataSize, s_CachedBytes, 0);
        }

        private void WriteStringData(int stringIndex, StringData stringData)
        {
            Utility.Marshal.StructureToBytes(stringData, StringDataSize, s_CachedBytes, 0);
            m_Stream.Position = m_StringDataOffset + StringDataSize * stringIndex;
