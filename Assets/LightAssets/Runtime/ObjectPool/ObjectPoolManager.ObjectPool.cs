...
namespace GameFrameworkAsset 
{
    internal sealed partial class ObjectPoolManager : IObjectPoolManager 
    {
        ...
        private sealed partial class ObjectPool<T> : AObjectPool, IObjectPool<T> where T : AObject
        {
            private readonly Dictionary<string, LinkedList<Object<T>>> m_Objects;
            private readonly Queue<LinkedListNode<Object<T>>> m_ObjectCaches;
            ...
            public ObjectPool(string name, bool allowMultiSpawn, float autoReleaseInterval, int capacity,
                ...
                m_ObjectCaches = new Queue<LinkedListNode<Object<T>>>();
            ...
            public bool CanSpawn(string name) {
                ...
                LinkedList<Object<T>> objectRange = default(LinkedList<Object<T>>);
            ...
            public bool ReleaseObject(object target) {
                ...
                RemoveObject(internalObject.Name, internalObject);
            ...
            private void AddObject(T obj, Object<T> internalObject) {
                if (!m_Objects.TryGetValue(obj.Name, out var linkedList)) {
                    linkedList = new LinkedList<Object<T>>();
                    linkedList.AddFirst(GetCacheNode(internalObject));
                    m_Objects.Add(obj.Name, linkedList);
                }
                else {
                    if (linkedList.First == null) {
                        ...
                        LinkedListNode<Object<T>> current = linkedList.First;
                        while (current != null) {
                        ...
                        linkedList.AddAfter(current, GetCacheNode(internalObject));
                    }

                    m_Objects[obj.Name] = linkedList;
                }
            }

            private LinkedListNode<Object<T>> GetCacheNode(Object<T> value) {
                if (m_ObjectCaches.Count > 0) {
                    LinkedListNode<Object<T>> node = m_ObjectCaches.Dequeue();
                    node.Value = value;
                    return node;
                }

                return new LinkedListNode<Object<T>>(value);
            }

            private bool RemoveObject(string name, Object<T> internalObject) 
            {
                if (m_Objects.TryGetValue(name, out var linkedList))
                {
                    LinkedListNode<Object<T>> current = linkedList.First;
                    while (current != null) 
                    {
                        if (current.Value.Equals(internalObject)) 
                        {
                            //删除的为头节点
                            ...
                            m_ObjectCaches.Enqueue(current);
                            return true;
                        }
